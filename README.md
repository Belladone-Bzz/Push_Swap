...
Our medium algorithm consists of dividing all the values into a number of √n chunks before computing them. This takes shape as a selection sort pushing the cloest value of the currently computed chunk repeatedly, until all values confined in said chunk are pushed over, and the next chunk is taken into account. When all chunks are moved over, the values are simply pushed back into the first stack by always accessing the highest value.
